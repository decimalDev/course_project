...
class Bus: public Public_transport{
	...
	void move(float time,sf::Clock &clock) override{
		...
		std::cout<<"Bus:"<<"move: traversed_path% = "<<traversed_path/sqrt((next_cross.point.x-cur_cross.point.x)*(next_cross.point.x-cur_cross.point.x) + (next_cross.point.y-cur_cross.point.y)*(next_cross.point.y-cur_cross.point.y))<<std::endl;//debug
